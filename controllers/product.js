...
router.post("/addToCart/:id", (req,res)=>{
    ...
    if (!res.locals.user) {
        ...
        .then((product)=>{
            ...
            const {_id, name, price, description, category, quantity, isBestseller, picture, dateCreated} = product;
...
router.get("/:id",(req,res)=>{
    ...
    .then((product)=>{
        ...
        if (numberFound == 1) {
            productInShoppingCartMessage.push(`${numberFound} unit is currently in your shopping cart.`);
        } else if (numberFound > 1){
            productInShoppingCartMessage.push(`${numberFound} units are currently in your shopping cart.`);
        }
...
router.post("/search",(req,res)=>{
    ...
    if (categorySearch == "All") {
        ...
        productModel.find({category:categorySearch})
        .then((products)=>{ 
...
router.get("/add/list", isAuthenticated, isClerk, (req,res)=>{
    ...
    productModel.find({createdBy:res.locals.user._id})
    .then((products)=>{ 
        ...
        const filteredProduct = products.map(product=>{
            ...
            return {
                ...
                isBestseller: product.isBestseller,
                picture: product.picture,
                dateCreated: moment(product.dateCreated).format('MMMM Do YYYY, h:mm:ss a')
            }
...
router.get("/delete/:id", isAuthenticated, isClerk, (req,res)=>{
    ...
    .catch(err=>console.log(`Error happened when deleting record in the database: ${err}`));
