...
router.post("/authentication",(req,res)=>{
    ...
    // in case the fields were properly filled, proceed checking credentials
        ...
        userModel.findOne({email: req.body.loginuserid})
        .then((user)=>{
                if (user == null) {
...
router.post("/registration",(req,res)=>{
    ...
    if(req.body.signupfirstname=="")
    {
            ...
            {
                    signupFirstNameErrorMessages.push("Your first name must be at least 2 characters long");
            }
            
            if(!nameStartRegex.test(req.body.signupfirstname))
            {
                    signupFirstNameErrorMessages.push("Your first name must begin with a letter");
            }
    }
    ...
    if(req.body.signuplastname=="")
    {
            ...
            {
                    signupLastNameErrorMessages.push("Your last name must be at least 2 characters long");
            }
            
            if(!nameStartRegex.test(req.body.signuplastname))
            {
                    signupLastNameErrorMessages.push("Your last name must begin with a letter");
            }
    }
    ...
    // in case the information was not properly enterered
    if(signupFirstNameErrorMessages.length > 0 || signupLastNameErrorMessages > 0 || signupEmailErrorMessages.length > 0 || 
        signupPasswordErrorMessages.length > 0 || signupPasswordConfirmationErrorMessages.length > 0) {

            const {signupfirstname, signuplastname, signupemail, signuppassword, signuppasswordconfirmation} =req.body;
        ...
        .then((user)=>{
                ...
                if (user != null) {
                        ...
                        signupErrorMessages.push("Account already exists - Please use a different e-mail address.");
                        ...
                        const msg = {
                        to: `${signupemail}`,
                        from: `accountcreation@mygiftshop.com`,
...
router.get("/shoppingCart",(req,res)=>{
        ...
        .then((products)=>{ 
                ...
                productModel.find().where('_id').in(idList)
                .then((products)=>{ 
                    ...
                    const productsInCart = products.map(product=>{
                        ...
                        for (let i = 0; i < productsInCart.length; i++) {
                                productsInCart[i].pricePerCartQuantity = (productsInCart[i].quantityInCart * productsInCart[i].price).toFixed(2);
                                orderSubtotal += (productsInCart[i].quantityInCart * productsInCart[i].price);
                        }

                        orderSubtotal = (orderSubtotal * 1.00).toFixed(2);

                        let shipping = (0 * 1.00).toFixed(2);
                        let tax = 0.13;
                        let taxAmount = parseFloat(orderSubtotal * tax).toFixed(2);
                        let orderTotal = (parseFloat(orderSubtotal) + parseFloat(shipping) + parseFloat(taxAmount)).toFixed(2);
...
router.put("/shoppingCart/update/:id", isAuthenticated, (req,res)=>{
        ...
        if (req.body.updateToSelectedQuantity < countOccurrences) {
                ...
                for (let i = 0; i < req.session.cartProducts.length; i++) {
                        if(req.session.cartProducts[i] == req.params.id && alreadySpliced < occurrencesToSplice) {
...
router.get("/placeOrder", isAuthenticated, (req,res)=>{
        ...
        .then((products)=>{ 
            ...
            const productsInCart = products.map(product=>{
                ...
                for (let i = 0; i < productsInCart.length; i++) {
                        orderDetailsToInsert.push(
                                `<tr>
                                                ...
                                                <td>$${parseFloat(productsInCart[i].price * productsInCart[i].quantityInCart).toFixed(2)}</td>
                                </tr>`);
                }
                ...
                from: `orderplaced@mygiftshop.com`,
